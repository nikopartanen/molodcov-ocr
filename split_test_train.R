...
gt_files <- dir("kpv_3-1691_1931_DATA/", 
                pattern = "xml", 
                recursive = TRUE,
                full.names = TRUE)
...
# gt_files %>%
#  .[5:6] %>%
#  walk(~ {alto2lines(xml_file = .x, target_directory = "train_1", output_textfile = ".alto.txt")})

# gt_files %>%
#  .[7:8] %>%
#  walk(~ {alto2lines(xml_file = .x, target_directory = "train_2", output_textfile = ".alto.txt")})
